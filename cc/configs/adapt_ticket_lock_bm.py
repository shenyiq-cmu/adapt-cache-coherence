...
system.cc = [AdaptCache(cache_id=i, 
                        serializing_bus=system.serializing_bus,
                        blockOffset=2,   
                        setBit=4,        # 16 sets
                        cacheSizeBit=10,
                        invalidThreshold=0,
                        invalidationRatio=2) # 1024 bytes total
             for i in range(N)]
...
print("Beginning simulation for Dragon protocol ticket lock test!")
...
if hasattr(system.serializing_bus, 'getTotalBusTransactions'):
    ...
    if hasattr(system.serializing_bus, 'getBusReadCount'):
        print(f"Bus Read Transactions: {system.serializing_bus.getBusReadCount()}")
    if hasattr(system.serializing_bus, 'getBusWriteCount'):
        print(f"Bus Write Transactions: {system.serializing_bus.getBusWriteCount()}")
    if hasattr(system.serializing_bus, 'getBusUpdCount'):
        print(f"Bus Update Transactions: {system.serializing_bus.getBusUpdCount()}")
...
for i in range(N):
    ...
    if hasattr(cache, 'updates'):
        print(f"Updates: {cache.updates}")